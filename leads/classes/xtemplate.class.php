...
function XTemplate ($file,$type="file",$mainblock="main") {
  ...
  switch($type){
  	...
  	break;
  	case "string":
  		$this->filecontents=$this->r_getfile2($file);
  	break;
  }
...
function parse ($bname) {
  ...
  $this->parsed_blocks[$bname].=chop($copy);
...
function text($bname) {
  ...
  //return chop(trim($this->parsed_blocks[$bname]));
  return trim($this->parsed_blocks[$bname]);
}
...
function out ($bname) {
  //echo chop(trim($this->text($bname)));
...
function render ($bname) {
  //return chop(trim($this->text($bname)));
...
function r_getfile($file) {
  ...
  while (preg_match($this->file_delim,$text,$res)) {
    ...
    $text=preg_replace("{".$res[0]."}",$text2,$text);
  }
  return $text;
}

function r_getfile2($file) {
  $text=$file;
  while (preg_match($this->file_delim,$text,$res)) {
    $text2=$this->getfile($res[1]);
    $text=preg_replace("{".$res[0]."}",$text2,$text);
